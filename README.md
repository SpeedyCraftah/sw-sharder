...
sw-sharder is a powerful sharding manager for the discord Eris library. It uses Node.js's cluster module to spread shards evenly among all the cores.

# Installation and usage
To download sw-sharder, run `npm install sw-sharder --save`

To use sw-sharder, simply copy this code and place it in a file, in the same directory that you ran the npm install in:

```javascript
const Sharder = require('sw-sharder').Master;
...
| `options.name` | Name to print on startup. By default it's "SW-Sharder".|
| `options.guildsPerShard` | Number to calculate how many guilds per shard. Defaults to 1300. Overriden if you only have 1 shard.|
| `options.nodeArgs` | Array of node arguments every cluster should run on. Defaults to `process.execArgv`
